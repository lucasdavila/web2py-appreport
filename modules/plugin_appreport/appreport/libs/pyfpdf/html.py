...
class HTML2FPDF(HTMLParser):
    ...
    def handle_data(self, txt):
        if self.td is not None: # drawing a table?
            #FIXME if 'width' not in self.td and 'colspan' not in self.td: #alterado para o codigo abaixo
            if 'width' not in self.td and 'colspan' not in self.td and \
                len(self.table_col_width) > 0 and not self.table_col_index > len(self.table_col_width) -1:

#                print '----'
#               print not self.table_col_index > len(self.table_col_width) -1
#                print 'self.table_col_width %s'%self.table_col_width
#                print 'self.table_col_index %s'%self.table_col_index
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag=='th':
            self.td = dict([(k.lower(), v) for k,v in attrs.items()])
            self.th = True
            #FIXME if self.td['width']: #alterado para o código abaixo (Lucas D'Avila)
            if 'width' in self.td:
