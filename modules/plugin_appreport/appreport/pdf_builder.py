...
class PdfBuilderPyfpdf(FPDF, HTMLMixin):
    ...
    def __init__(self, report):
        ...
        FPDF.__init__(self, orientation = self.report.orientation, unit = self.report.unit, 
            format = self.report.format)

        self.set_title(title = self.report.title)
    ...
    def header(self):
        ...
        self.cell(w = len(self.report.title), txt = self.report.title, align = 'C')
        self.ln(5)
